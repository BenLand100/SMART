...
void exp_setMouseInput(bool enabled) {
    if (local) {
        *(bool*)(local->data->args) = enabled;
        callClient(local,setMouseInput);
    }
}
...
bool exp_isKeysEnabled() {
    if (local) {
        callClient(local,isKeyboardEnabled);
        return *(bool*)(local->data->args);
    } else return false;
}
...
bool exp_isBlocking() {
    if (local) {
        callClient(local,isMouseEnabled);
        return *(bool*)(local->data->args);
    } else return false;
}
...
void exp_setMode(int mode) {
    if (local) {
        *(int*)(local->data->args) = mode;
        callClient(local,setMode);
    }
}
